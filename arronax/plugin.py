...
class Plugin(GObject.GObject):
    ...
    def _create_starter_for(self, path):
        ...
        func =  lambda *x: editor.Editor(
            path=path, mode=editor.MODE_CREATE_FOR)
        return label, func

    def _create_starter_edit(self, path):
        label = _('Modify this starter')
        func =  lambda *x: editor.Editor(
            path=path, mode=editor.MODE_EDIT)
        return label, func

    def _create_starter_in(self, path):
        label = _('Create a starter')
        func =  lambda *x: editor.Editor(
            path=path, mode=editor.MODE_CREATE_IN)
        return label, func

    def _create_starter_link(self, path):
        label = _('Create a starter for this file')
        func =  lambda *x: editor.Editor(
            path=path, mode=editor.MODE_CREATE_FOR, type=editor.TYPE_LINK)
        return label, func
